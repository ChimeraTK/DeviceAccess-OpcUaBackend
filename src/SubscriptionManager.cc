...
namespace ChimeraTK {
  ...
  void OPCUASubscriptionManager::activate() {
    if(!_subscriptionActive) createSubscription();
  ...
  void OPCUASubscriptionManager::addMonitoredItems() {
    _mutex.lock();
    for(auto& item : _items) {
      if(_asyncReadActive) item._active = true;
      if(!item._isMonitored) {
        ...
        if(monResponse.statusCode == UA_STATUSCODE_GOOD) {
          ...
          _mutex.unlock();
          handleException(std::string("Failed to add monitored item for node: ") + item._browseName +
