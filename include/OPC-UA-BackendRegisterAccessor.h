...
namespace ChimeraTK {
  ...
  class RangeCheckingDataConverter {
   ...
   public:
    static DestType convert(SourceType& x) {
      try {
  ...
  class RangeCheckingDataConverter<UA_String, SourceType> {
   public:
    static UA_String convert(SourceType& x) { return UA_STRING((char*)std::to_string(x).c_str()); }
  };

  template<typename SourceType>
  class RangeCheckingDataConverter<std::string, SourceType> {
   public:
    static std::string convert(SourceType& x) { return std::to_string(x); }
  };

  // partial specialization of conversion to bool
  template<typename DestType>
  class RangeCheckingDataConverter<DestType, ChimeraTK::Boolean> {
   public:
    static DestType convert(ChimeraTK::Boolean& x) { return x == 0 ? DestType(false) : DestType(true); }
  };

  template<>
  class RangeCheckingDataConverter<UA_String, ChimeraTK::Boolean> {
   public:
    static UA_String convert([[maybe_unused]] ChimeraTK::Boolean& x) {
      return x == 0 ? UA_String_fromChars("false") : UA_String_fromChars("true");
    }
  };
  ...
  class RangeCheckingDataConverter<DestType, Void> {
   public:
    static DestType convert([[maybe_unused]] Void& x) { return DestType(); } // default constructed value
  };
  ...
  class RangeCheckingDataConverter<Void, SourceType> {
   public:
    static Void convert([[maybe_unused]] SourceType& x) { return Void(); }
  };
  ...
  class RangeCheckingDataConverter<UA_String, std::string> {
   public:
    static UA_String convert(std::string& x) { return UA_String_fromChars(x.c_str()); }
  };
  ...
  class RangeCheckingDataConverter<DestType, UA_String> {
   public:
    static DestType convert(UA_String&) { throw std::logic_error("Conversion from string is not allowed."); }
  };
  template<typename DestType>
  class RangeCheckingDataConverter<DestType, std::string> {
   public:
    static DestType convert(std::string&) { throw std::logic_error("Conversion from string is not allowed."); }
  };

  template<>
  class RangeCheckingDataConverter<std::string, UA_String> {
   public:
    static std::string convert(UA_String& x) { return std::string((char*)x.data, x.length); }
  };
  ...
  class RangeCheckingDataConverter<UA_String, ChimeraTK::Void> {
   public:
    static UA_String convert([[maybe_unused]] ChimeraTK::Void& x) { return UA_String_fromChars("void"); }
  };
  ...
  class RangeCheckingDataConverter<ChimeraTK::Void, UA_String> {
   public:
    static ChimeraTK::Void convert([[maybe_unused]] UA_String& x) { return ChimeraTK::Void(); }
  };
  ...
  class OpcUABackendRegisterAccessorBase {
   public:
    ...
    bool subscribed{false}; ///< Remember if a subscription was added.
  ...
  class OpcUABackendRegisterAccessor : public OpcUABackendRegisterAccessorBase, public NDRegisterAccessor<CTKType> {
   public:
    ...
    bool isReadOnly() const override { return info->isReadonly; }

    bool isReadable() const override { return true; }

    bool isWriteable() const override { return !info->isReadonly; }
    ...
    size_t numberOfWords; ///< Requested array length. Could be smaller than what is available on the server.
    size_t offsetWords;   ///< Requested offset for arrays.
  ...
  OpcUABackendRegisterAccessor<UAType, CTKType>::OpcUABackendRegisterAccessor(const RegisterPath& path,
      boost::shared_ptr<DeviceBackend> backendBase, const std::string& node_id, OpcUABackendRegisterInfo* registerInfo,
      AccessModeFlags flags, size_t numberOfWords, size_t wordOffsetInRegister)
  : OpcUABackendRegisterAccessorBase(boost::dynamic_pointer_cast<OpcUABackend>(backendBase), registerInfo),
    NDRegisterAccessor<CTKType>(path, flags), node_id(node_id), numberOfWords(numberOfWords),
    offsetWords(wordOffsetInRegister) {
    ...
    if(flags.has(AccessMode::wait_for_new_data)) {
      ...
      if(!backend->_subscriptionManager) {
        backend->activateSubscriptionSupport();
      }
      backend->_subscriptionManager->subscribe(info->nodeBrowseName, info->id, this);
    ...
    if(info->arrayLength != numberOfWords) {
      isPartial = true;
    }
  ...
  void OpcUABackendRegisterAccessor<UAType, CTKType>::doReadTransferSynchronously() {
    ...
    UA_StatusCode retval = UA_Client_readValueAttribute(backend->_connection->client.get(), info->id, val->var);
  ...
  bool OpcUABackendRegisterAccessor<UAType, CTKType>::doWriteTransfer(ChimeraTK::VersionNumber versionNumber) {
    ...
    if(retval == UA_STATUSCODE_BADNOTWRITABLE || retval == UA_STATUSCODE_BADWRITENOTSUPPORTED) {
      if(backend->_subscriptionManager) {
        backend->_subscriptionManager->setExternalError(info->nodeBrowseName);
      }
      throw ChimeraTK::logic_error(std::string("OPC-UA-Backend::Variable ") + node_id + " is not writable!");
    }
    handleError(retval);
    return false;
  }
  ...
  void OpcUABackendRegisterAccessor<UAType, CTKType>::handleError(const UA_StatusCode& retval) {
    ...
    out << "OPC-UA-Backend::Failed to access variable: " << node_id << " with reason: " << UA_StatusCode_name(retval)
        << " --> " << std::hex << retval;
    if(backend->_subscriptionManager) {
      backend->_subscriptionManager->setExternalError(info->nodeBrowseName);
    }
